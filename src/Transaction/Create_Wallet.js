...
class Create_Wallet extends React.Component {
    ...
    handleSubmit() {
        if (this.state.balance_error == "ready") {
            ...
            MaxUniqueId = Math.max.apply(
                Math,
                AllWallet.map(function(item) {
                    return item.id_wallet;
                })
            );
            MaxUniqueId++;

            var newWallet = {
                id_wallet: MaxUniqueId,
    ...
    render() {
        ...
        const DepositIsExist = DataRequest.get_current_transfert(
            "money_deposit",
            walletIsExist
        );
        const WithDrawalIsExist = DataRequest.get_current_transfert(
            "money_withdrawal",
            walletIsExist
        );
        const TransitIsExist = DataRequest.get_current_transfert(
            "money_transit",
            walletIsExist
        );
            ...
            <div className="container">
                            ...
                            <Card style={{ width: "18rem" }}>
                                <Card.Body>
                                    ...
                                    <Card.Subtitle className="mb-2 text-muted">
                                        {walletIsExist == undefined ? (
                                            <p>
                                                This wallet will be attributed
                                                to :{" "}
                                                <strong>
                                                    {" "}
                                                    {
                                                        this.state.actual_user
                                                            .first_name_users
                                                    }{" "}
                                                    {
                                                        this.state.actual_user
                ...
                {walletIsExist != undefined ? (
                    <Row>
                        ...
                        <Col xs={12}>
                            <MyMonnaie
                                wallet={walletIsExist}
                                isExist={DepositIsExist}
                                name="money_deposit"
                        ...
                        <Col xs={12}>
                            <MyMonnaie
                                wallet={walletIsExist}
                                AllNameUsers={AllNameUsers}
                                isExist={WithDrawalIsExist}

  ...
  class Authentification extends React.Component {
   ...
   onSubmit(e) {
    ...
    const UserIsExist = this.state.usersExist.filter(function (item) {
      ...
      if(user.email_users == item.email_users
    ...
    if (UserIsExist.length < 1){
      ...
      this.setState({formErrors : {formErrors : "This user does not exist."}});

    }else
    {

      localStorage.setItem("connected_user", JSON.stringify(UserIsExist[0]));
  ...
  render() {
    ...
    return (
      ...
      { this.state.user_connect ? <h3> You are already logged ! </h3> :
        ...
        <input className="send_info" type="submit" value="Login"/>
        </div>
        </Col>

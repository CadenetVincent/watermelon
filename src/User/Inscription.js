...
class Inscription extends React.Component {
    ...
    validateForm() {
        this.setState({
            allValid:
                this.state.formErrors.first_name_users == "ready" &&
    ...
    validateField(fieldName, value) {
        ...
        formErrors = DataRequest.validate_field_user(
            fieldName,
            value,
            formErrors,
            user
    ...
    onSubmit(e) {
        ...
        const UserIsExist = this.state.usersExist.filter(function(item) {
            if (
                (user.first_name_users == item.first_name_users &&
        ...
        if (UserIsExist.length > 0) {
            this.setState({
                formErrors: {
                    ...
                    is_exist: "This user already exists."
                }
            });
        } else if (this.state.allValid == true) {
            MaxUniqueId = Math.max.apply(
                Math,
                this.state.usersExist.map(function(item) {

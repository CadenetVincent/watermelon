...
export function get_list_user()
{
 ...
 const result = actual_user.is_admin_users ? parsing_user_file() : "undefined";
...
export function get_current_card(value_url)
{
  ...
  {
    result = actual_user.is_admin_users ? list_cards_by_user_id : "undefined";
  }
...
export function get_all_users_name()
{
  ...
  {
    result[0] = actual_user.first_name_users+" "+actual_user.last_name_users;
...
export function right_access_update_card(id)
{
  ...
  if(actual_user.id_users == card.user_id || actual_user.is_admin_users == true)
  {
    return true;
  }
...
export function validate_field_card(fieldName,value,formErrors)
{
  ...
  switch(fieldName) {
    ...
    if(value.length > 22){ formErrors.number = 'Credit card number is too long.'; }
    if(value.length < 16){ formErrors.number = 'Credit card number is too short.'; }

    formErrors.number = regex_digit_only_nbr.test(value) ? formErrors.number : 'Only digit is accepted.';
    ...
    formErrors.name = value != "undefined" ? formErrors.name : 'You must to select a name.';
    ...
    if(value.length > 4){ formErrors.cvc = 'CVC is too long.'; }
    if(value.length < 3){ formErrors.cvc = 'CVC is too short.'; }

    formErrors.cvc = regex_digit_only_cvc.test(value) ? formErrors.cvc : 'Only digit is accepted.';
    ...
    formErrors.card_type = value != "undefined" ? formErrors.card_type : 'You must to select a card type.';
    ...
    formErrors.bank_name = value != "undefined" ? formErrors.bank_name : 'You must to select a bank name.';

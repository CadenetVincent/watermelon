...
class Cards extends Component {
    constructor(props) {
        super(props);
        this.state = { Card: [], Current_card: [], is_admin: false };
    ...
    componentDidMount() {
        this.setState({
            Card: DataRequest.get_current_card(this.props.match.params.value)
        });
    }
    ...
    tabRow() {
        ...
        for (var i = 0; i < this.state.Card.length; i++) {
            mapping.push(
                <Card delete={this.delete} obj={this.state.Card[i]} key={i} />
            );
        }
